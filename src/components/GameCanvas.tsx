...
import { GameState, Enemy, Bullet, GoldCoin, UpgradeOption, Crosshair } from '../types';
...
const GameCanvas: React.FC = () => {
  ...
  const gameLoop = (time: number) => {
    ...
    if (state.gameStarted && !state.isPaused && !state.isGameOver && !state.showPowerUpMenu && !state.showPauseMenu) {
      ...
      if (state.isMultiplayer && state.player2) {
        ...
        if (moveX2 !== 0 || moveY2 !== 0) {
          ...
          newState.player2.x = Math.max(state.player2.radius, Math.min(canvas.width - state.player2.radius, state.player2.x + (moveX2 / dist) * state.player2.speed));
          newState.player2.y = Math.max(state.player2.radius, Math.min(canvas.height - state.player2.radius, state.player2.y + (moveY2 / dist) * state.player2.speed));
        }
      }
      ...
      // Player 2 (Auto-shoot nearest enemy in multiplayer)
      if (state.isMultiplayer && state.player2 && time - state.player2.lastShotTime > state.player2.attackSpeed) {
        let nearestEnemy: Enemy | null = null;
        let minDist = Infinity;

        state.enemies.forEach(e => {
          const dist = Math.sqrt((e.x - state.player2!.x)**2 + (e.y - state.player2!.y)**2);
          if (dist < minDist) {
            minDist = dist;
            nearestEnemy = e;
          }
        ...
        if (nearestEnemy) {
          const angle = Math.atan2(nearestEnemy.y - state.player2.y, nearestEnemy.x - state.player2.x);
          newState.bullets.push({
            id: Math.random().toString(36).substr(2, 9),
            x: state.player2.x,
            y: state.player2.y,
            ...
            vx: Math.cos(angle) * state.player2.bulletSpeed,
            vy: Math.sin(angle) * state.player2.bulletSpeed,
            damage: state.player2.damage,
            life: 100
          });
          newState.player2.lastShotTime = time;
        }
      }
      ...
      newState.enemies = state.enemies.map(e => {
        ...
        if (state.isMultiplayer && state.player2) {
          const dist1 = Math.sqrt((state.player.x - e.x)**2 + (state.player.y - e.y)**2);
          const dist2 = Math.sqrt((state.player2.x - e.x)**2 + (state.player2.y - e.y)**2);
          if (dist2 < dist1) targetPlayer = state.player2;
        }
      ...
      newState.enemies.forEach(e => {
        ...
        if (state.isMultiplayer && state.player2) {
          const dist2 = Math.sqrt((e.x - state.player2.x)**2 + (e.y - state.player2.y)**2);
          if (dist2 < e.radius + state.player2.radius) {
            newState.player2.hp -= 0.5;
          }
        }
      });
      ...
      newState.goldCoins.forEach((coin, idx) => {
        // Magnet effect for both players
        let nearestPlayer = state.player;
        let minDist = Math.sqrt((coin.x - state.player.x)**2 + (coin.y - state.player.y)**2);
        
        if (state.isMultiplayer && state.player2) {
          const dist2 = Math.sqrt((coin.x - state.player2.x)**2 + (coin.y - state.player2.y)**2);
          if (dist2 < minDist) {
            minDist = dist2;
            nearestPlayer = state.player2;
          }
        }
        ...
        if (minDist < coin.radius + nearestPlayer.radius) {
          ...
          if (newPoints > oldPoints) {
            ...
            if (state.isMultiplayer && newState.player2) {
              newState.player2.upgradePoints += pointsToAdd;
            }
          }

          if (state.isMultiplayer && newState.player2) {
            newState.player2.totalXp = newState.player.totalXp;
          }
      ...
      if (state.isMultiplayer && newState.player2 && newState.player2.hp < newState.player2.maxHp) {
        newState.player2.hp = Math.min(newState.player2.maxHp, newState.player2.hp + newState.player2.regenRate);
      }
      ...
      // Game Over
      if (newState.player.hp <= 0 || (state.isMultiplayer && newState.player2 && newState.player2.hp <= 0)) {
    ...
    if (state.isMultiplayer && state.player2) {
      ...
      ctx.shadowBlur = 20;
      ctx.shadowColor = state.player2.color;
      ctx.beginPath();
      ctx.arc(state.player2.x, state.player2.y, state.player2.radius, 0, Math.PI * 2);
      ctx.fill();

      ctx.fillStyle = 'white';
      ctx.font = '10px Inter';
      ctx.textAlign = 'center';
      ctx.fillText('Player 2', state.player2.x, state.player2.y - state.player2.radius - 10);
    }
  ...
  const applyUpgrade = (type: 'damage' | 'speed' | 'attackSpeed' | 'regen', playerIndex: 1 | 2 = 1) => {
    setGameState(prev => {
      const targetPlayer = playerIndex === 1 ? prev.player : prev.player2;
  ...
  const selectPlayerSkin = (id: string) => {
    ...
    if (skin && (skin.requiredWave === 0 || stateRef.current.maxWaveReached >= skin.requiredWave)) {
      setGameState(prev => ({
        ...
        player2: prev.player2 ? { ...prev.player2, color: skin.color, skinId: id } : null
      }));
    }
  };
      ...
      <div className="absolute top-4 left-4 right-4 flex justify-between items-start pointer-events-none">
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <div className="flex flex-col items-end gap-3 w-48">
            ...
            >
              <Zap className={`w-6 h-6 ${(gameState.player.upgradePoints > 0 || (gameState.isMultiplayer && gameState.player2 && gameState.player2.upgradePoints > 0)) ? 'text-yellow-400 animate-pulse' : 'text-white/40'}`} />
              <span className="text-[10px] font-black uppercase tracking-tighter text-center leading-tight">Güçlendirme<br/>Menüsü</span>
              {(gameState.player.upgradePoints > 0 || (gameState.isMultiplayer && gameState.player2 && gameState.player2.upgradePoints > 0)) && (
                <div className="absolute -top-2 -right-2 bg-red-500 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center shadow-lg">
                  {gameState.isMultiplayer && gameState.player2 
                    ? gameState.player.upgradePoints + gameState.player2.upgradePoints 
            ...
            <div className="flex flex-col items-end">
              ...
              {gameState.isMultiplayer && gameState.player2 ? (
                <div className="flex flex-col items-end">
                  <span className="text-green-400 font-mono text-[9px] uppercase tracking-widest">P1 Hak: {gameState.player.upgradePoints}</span>
                  <span className="text-pink-400 font-mono text-[9px] uppercase tracking-widest">P2 Hak: {gameState.player2.upgradePoints}</span>
                </div>
              ) : (
          ...
          >
            ...
            >
              ...
              <div className="flex gap-8">
                ...
                {/* Player 2 Menu (Multiplayer only) */}
                {gameState.isMultiplayer && gameState.player2 && (
                  <>
                    ...
                    <div className="flex-1">
                      <div className="text-center mb-6">
                        <h2 className="text-4xl font-black text-white italic uppercase tracking-tighter">GÜÇLENDİRME</h2>
                        <p className="text-pink-400 font-bold text-xs tracking-widest uppercase mt-1">PLAYER 2</p>
                        <div className="flex items-center justify-center gap-2 mt-2">
                          <span className="text-white/40 text-sm uppercase tracking-widest">Hak:</span>
                          <span className="text-yellow-400 font-black text-xl">{gameState.player2.upgradePoints}</span>
                        </div>
                      </div>

                      <div className="space-y-3">
                        {UPGRADE_OPTIONS.map((u) => {
                          const level = gameState.player2!.upgradeLevels[u.id as keyof typeof gameState.player.upgradeLevels];
                          const canUpgrade = gameState.player2!.upgradePoints > 0;
                          ...
                          return (
                            <div key={u.id} className="flex items-center gap-4 bg-white/5 border border-white/10 p-3 rounded-2xl">
                              ...
                              <div className="flex items-center gap-3">
                                <button
                                  onClick={() => applyUpgrade(u.id as any, 2)}
                                  disabled={!canUpgrade}
                                  className={`w-9 h-9 rounded-xl flex items-center justify-center transition-all ${
                                    canUpgrade 
